...
const validateForm = () => {
    ...
    if (!form.phoneNumber.trim()) {  
        ...
        // Clean the phone number by removing leading zeros and non-digits
        const cleanedNumber = form.phoneNumber.replace(/[^\d]/g, '').replace(/^0+/, '');
        if (cleanedNumber.length < 7 || cleanedNumber.length > 15) {
            newErrors.phoneNumber = t('auth.phoneInvalid', 'Please enter a valid phone number (7-15 digits)');  
        }
    }  
...
const handleSubmit = async (e: React.FormEvent) => {  
    ...
    try {  
        // Clean and normalize the phone number - remove all non-digits, then remove leading zeros
        const cleanedNumber = form.phoneNumber.replace(/[^\d]/g, '').replace(/^0+/, '');
        
        // If after cleaning there's no number left, use the original (edge case)
        ...
        console.log('Sign-up: Attempting registration with:', {   
            ...
            originalInput: form.phoneNumber,
