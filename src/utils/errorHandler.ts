...
export function handleGlobalError(error: unknown): string {
  ...
  return (error instanceof Error ? error.message : 'Registration failed. Please try again.') || 'Registration failed. Please try again.';
}
...
if (typeof window !== 'undefined') {
  window.addEventListener('error', (event) => {
    ...
    if (event.error instanceof Error && event.error.message === 'Something went wrong') {
  ...
  window.addEventListener('unhandledrejection', (event) => {
    ...
    if (event.reason instanceof Error && event.reason.message === 'Something went wrong') {
      event.preventDefault();
